...
type DoseSlot = z.infer<typeof doseSlotEnum>;

const SLOT_HOURS_UTC: Record<DoseSlot, number> = {
  MORNING: 8,
  AFTERNOON: 13,
  EVENING: 19,
  NIGHT: 22,
...
function defaultSlotsFromFrequency(frequency?: string | null): DoseSlot[] {
  const f = (frequency ?? "").toLowerCase();

  // Try common numeric patterns first.
  const numMatch = f.match(/\b([1-4])\b/);
  const n = numMatch ? Number(numMatch[1]) : null;
  if (n === 4) return ["MORNING", "AFTERNOON", "EVENING", "NIGHT"];
  ...
  if (f.includes("morning")) slots.push("MORNING");
  if (f.includes("afternoon")) slots.push("AFTERNOON");
  if (f.includes("evening")) slots.push("EVENING");
  if (f.includes("night") || f.includes("bed")) slots.push("NIGHT");
...
function listDueDoseTimes(args: { last: Date; now: Date; slots: DoseSlot[] }): Date[] {
...
async function reconcileMedicinesForPatient(patientId: string, now: Date): Promise<void> {
  ...
  for (const m of medicines) {
    ...
    const slots = (Array.isArray(slotsRaw) ? slotsRaw : [])
      .map((s) => String(s))
      .filter((s): s is DoseSlot => (doseSlotEnum.options as string[]).includes(s));
...
async function assertCanAccessPatientData(actorId: string, targetPatientId: string): Promise<void> {
  if (actorId === targetPatientId) return;
  ...
  if (!link) throw new HttpError(403, "Not authorized");
}
...
app.post("/family/requests", async (req, res) => {
  ...
  if (!memberProfile) throw new HttpError(404, "Patient not found");
  const memberId = memberProfile.userId;
  if (memberId === ownerId) throw new HttpError(400, "Cannot add yourself");
...
app.post("/emergency/contacts", async (req, res) => {
  ...
  if (body.uniquePatientId) {
    ...
    if (profile.userId === patientId) throw new HttpError(400, "Cannot add yourself");
...
app.patch("/emergency/family-links/:linkId", async (req, res) => {
  ...
  if (link.memberId !== memberId) throw new HttpError(403, "Not authorized");
  if (link.status !== "ACCEPTED") throw new HttpError(400, "Link not accepted");
...
app.post("/family/requests/:linkId/accept", async (req, res) => {
  ...
  const link = await prisma.familyLink.findUnique({ where: { id: linkId } });
  if (!link) throw new HttpError(404, "Request not found");
  if (link.memberId !== memberId) throw new HttpError(403, "Not authorized");
...
app.delete("/family/links/:linkId", async (req, res) => {
  ...
  const link = await prisma.familyLink.findUnique({ where: { id: linkId } });
  if (!link) throw new HttpError(404, "Link not found");
  if (link.ownerId !== userId && link.memberId !== userId) throw new HttpError(403, "Not authorized");
...
app.get("/files/:fileUploadId/download", async (req, res) => {
  ...
  if (!uploadRecord) throw new HttpError(404, "File not found");
  ...
  if (!isHospital && uploadRecord.patientId !== userId) {
    if (isDoctor) {
      ...
      if (!share) throw new HttpError(403, "Not authorized to download this file");
    } else {
  ...
  res.setHeader(
    "Content-Disposition",
    `attachment; filename="${encodeURIComponent(uploadRecord.originalName)}"`
  );
...
app.get("/files/:fileUploadId/view", async (req, res) => {
  ...
  if (!isHospital && uploadRecord.patientId !== userId) {
    if (isDoctor) {
      ...
      if (!share) throw new HttpError(403, "Not authorized to view this file");
    } else {
      await assertCanAccessPatientData(userId, uploadRecord.patientId);
    }
  }
...
app.post("/share-requests", async (req, res) => {
  ...
  if (body.fileUploadIds.length > 0) {
    ...
    if (count !== body.fileUploadIds.length) throw new HttpError(400, "One or more files do not belong to patient");
  }
...
app.get("/doctor/inbox/:requestId", async (req, res) => {
  ...
  if (request.doctorId !== doctorId) throw new HttpError(403, "Not authorized");
  ...
  items: z
    .array(
      z.object({
        ...
        slots: z.array(z.enum(["MORNING", "AFTERNOON", "EVENING", "NIGHT", "BEDTIME"])).optional(),
...
app.post("/doctor/inbox/:requestId/prescriptions", async (req, res) => {
      ...
      items: {
        createMany: {
          data: body.items.map((it) => ({
            ...
            dosage: it.dosage,
            dosageAmount: it.dosageAmount ?? null,
            dosageUnit: it.dosageUnit ?? null,
            frequency: it.frequency ?? (it.slots ? it.slots.join(", ") : undefined),
            slots: it.slots ? JSON.stringify(it.slots) : null,
            duration: it.duration ?? (it.durationDays ? `${it.durationDays} days` : undefined),
  ...
  for (const itemInput of body.items) {
    const prescriptionItem = prescription.items.find((pi) => pi.medicineName === itemInput.medicineName);
    if (!prescriptionItem) continue;
    // Use explicitly chosen slots if provided, else parse from frequency text.
    const slotsForMed = itemInput.slots && itemInput.slots.length > 0
      ? itemInput.slots
      : defaultSlotsFromFrequency(itemInput.frequency);
    await prisma.patientMedicine.create({
      data: {
        patientId: targetPatientId,
        prescriptionItemId: prescriptionItem.id,
        medicineName: prescriptionItem.medicineName,
        dosageText: prescriptionItem.dosage,
        frequencyText: prescriptionItem.frequency,
        durationText: prescriptionItem.duration,
        slotsJson: slotsForMed,
        unitsPerDose: 1,
        inventoryCount: 0,
        lastReconciledAt: now,
      },
...
app.get("/medicines/current", async (req, res) => {
  ...
  if (user?.role !== "PATIENT") throw new HttpError(403, "Not authorized");
...
app.post("/medicines/:medicineId/adjust", async (req, res) => {
  ...
  if (!med || med.patientId !== patientId) throw new HttpError(404, "Medicine not found");
...
app.get("/patients/me/prescriptions/:prescriptionId", async (req, res) => {
  ...
  if (prescription.patientId !== patientId) throw new HttpError(403, "Not authorized");
...
app.get("/patients/:patientId/prescriptions/:prescriptionId", async (req, res) => {
  ...
  if (!prescription) throw new HttpError(404, "Prescription not found");
  if (prescription.patientId !== patientId) throw new HttpError(404, "Prescription not found");
...
app.get("/hospital/patients/:uniquePatientId", async (req, res) => {
  const userId = getUserId(req);
  const user = await prisma.user.findUnique({ where: { id: userId } });
  if (user?.role !== "HOSPITAL") throw new HttpError(403, "Not authorized");
  ...
  void (async () => {
    try {
      ...
      for (const c of contacts) {
        if (c.kind === "EMAIL" && c.email) emails.push(c.email);
        if (c.kind === "USER") {
...
const vitalCreateSchema = z.object({
  ...
  bloodSugarFasting: z.number().min(0).max(2000).optional(),
  bloodSugarPostMeal: z.number().min(0).max(2000).optional(),
...
app.post("/vitals", async (req, res) => {
  const userId = getUserId(req);
  const body = vitalCreateSchema.parse(req.body);
  ...
  const vital = await prisma.vitalRecord.create({
    data: {
      ...
      bloodSugarFasting: body.bloodSugarFasting ?? null,
      bloodSugarPostMeal: body.bloodSugarPostMeal ?? null,
...
app.get("/vitals", async (req, res) => {
  const userId = getUserId(req);
  const limit = Math.min(500, Number(req.query.limit ?? 90));
...
app.get("/vitals/latest", async (req, res) => {
  const userId = getUserId(req);

  const latest = await prisma.vitalRecord.findFirst({
    where: { patientId: userId },
    orderBy: { recordedAt: "desc" },
  });

  res.json({ ok: true, vital: latest ?? null });
});
...
app.delete("/vitals/:id", async (req, res) => {
  ...
  const result = await prisma.vitalRecord.deleteMany({
    where: { id, patientId: userId },
  });

  if (result.count === 0) throw new HttpError(404, "Vital record not found");
  res.json({ ok: true });
});
